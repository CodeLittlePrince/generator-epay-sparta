...
// sdk日志调试类，连接Hubble平台
/**
 * 1. 实时调试功能模块(https://nei.netease.com/interface/detail/req/?pid=18835&id=149910) 
 * 2. 日志调试数据上报
 * 
 * 说明： 实时调试模块-- 启动小程序时获取到凭证后开启，数据上报调用指定接口
 *        日志数据上报到平台-- 正常上报数据中新增 公共字段 isDebug =1表示调试模式
 */
class Debug {
    ...
    // 上报实时日志到平台
    log(data) {
      const url = this.DATracker.getConfig('debugHost') + '/debug/track?token=' + this._token + '&userId=' + (this.DATracker.getUserId() || this.DATracker.getDeviceUdid());
      this.DATracker.SDKAPI.request({
        url: url,
        data: data || {},
        method: "POST"
      });
 ...
 * promotional_id : p_id
 */
// 推广参数 B 接口获取的参数格式：aaa*bbb*ccc**dd 
class Campaign {
    constructor(instance, query, _) {
      ...
      // 本地保存数据key ：DATracker_wechat_u
      this._key = 'DATracker_wechat_u';
      // 本地缓存超时时间（单位，天）
      this._day = 30;
      if (!instance) {
        return;
    ...
    _setLocal() {
      ...
      this.DATracker.SDKAPI.setStorageSync(this._key, str);
    }
    // 获取本地缓存数据
    _getLocal() {
      const str = this.DATracker.SDKAPI.getStorageSync(this._key);
      try {
...
class DATrackAgent {
    ...
    constructor(instance, _) {
      ...
      if (this.DATracker.getConfig('miniType') == 'swan') {
        swan.HubbleAppLaunchOk = false;
      }
    ...
    appLaunch(e) {
      e.query =  e.query || {};
      const DATracker = this.DATracker;  
      ...
      const fn1 = () => {
        ...
        DATracker.setScene(e.scene);
        DATracker.setReferrer('');
        DATracker.setOldReferrer('');
        // debug参数（实时调试） scene
        new Debug(DATracker, sceneDebugConf);
        // 渠道推广
        new Campaign(DATracker, campaign.campaign, this._);
        DATracker.daActivate();
  
        DATracker['campaign'].init();
  
        let pros = {};
        // 设置分享相关的字段值
        DATracker.setShareInfo(e, pros);
        // 触发da_onLaunch 事件，应用唤醒的时候触发
        DATracker.trackOnLaunch(pros);
      };

      fn1();

      // 百度小程序：appShow 比 appLaunch 触发的早 
      // if (DATracker.getConfig('miniType') == 'swan') {
      //   swan.HubbleAppLaunchOk = true;
      //   fn2();
      // }
    }
    appShow(e) {
    ...
    pageUnload() {
      this.DATracker.setReferrer();
      this.DATracker.set();
    }
    pageShow(e, eventName, targetPage) {
      const DATracker = this.DATracker;
      const onPageShowFn = DATracker.getConfig('onPageShow');
      let route = '';
      // 获取当前页面path
      ...
      DATracker.setUrl();
      DATracker.curPageObj = targetPage;
      if(typeof onPageShowFn === 'function') {
        onPageShowFn(DATracker, route, targetPage);
      } else {
    ...
    pageShareAppMessage(e) {
      this.DATracker.track_share({}, e && e.channel);
    }
    ...
    Page(obj) {
      ...
      if(
        DATracker.getConfig('shareAll') && 
        !DATracker.getConfig('allowAmendSharePath') &&
          !DATracker.getConfig('share')
      ) {
      ...
      if (typeof obj.onShareAppMessage === 'function') {
        ...
        obj.onShareAppMessage = (e) => {
          ...
          const firstShareFn = (sharePath) => {
            let first = true;
            if (
              DATracker.firstShareUid !== 'notFromShare' &&
              DATracker.lastShareUid !== 'notFromShare' &&
              DATracker.shareUrlPath !== 'notFromShare' &&
              DATracker.shareDepth !== 'notFromShare'
            ) {
              first = false;
            }
            if (sharePath) {
              if (sharePath != DATracker.shareUrlPath) {
                first = true;
              }
            }
            return first;
          ...
          let shareInfo = old.apply(DATracker.curPageObj, arguments);
          // 用户配置了允许sdk更改分享path
          if (DATracker.getConfig('allowAmendSharePath')) {
            // localshareInfo 为本次需分享的参数信息
            let localshareInfo = DATracker.getShareInfo();
            // 检测用户填写的是否合法的path，是的重新设置本次分享的path（参数hubbleshare= xxx 后面的值中path）
            if (
            ...
            if (firstShare) {
              localshareInfo.$firstShareUid = DATracker.getUserId() || DATracker.getDeviceUdid();
              localshareInfo.$lastShareUid = DATracker.getUserId() || DATracker.getDeviceUdid();
            ...
            const daShareInfo = {
              ...
              $lastShareUid: firstShare ? 'notFromShare' : DATracker.getLastShareUid(),
              $shareDepth: localshareInfo.$shareDepth
            };
            // sdk自动上报da_share事件
            DATracker.track_share(daShareInfo, e && e.channel);
            ...
            if (
              ...
              shareInfo.path = shareInfo.path + "hubbleshare=" + encodeURIComponent(JSON.stringify(localshareInfo));
            }
          } else {
            // 配置了自定义分享，但不进行分享后续追踪
            if (DATracker.getConfig('share')) {
              // sdk自动上报da_share事件
              DATracker.track_share({}, e && e.channel);
            }
          }
...
const _ = {
  ...
  localStorage: {
    ...
    setStorage(str) {
      this.SDKAPI.setStorageSync('DATracker_wechat', str);
    },
    getStorage() {
      return this.SDKAPI.getStorageSync('DATracker_wechat') || "";
    },
...
class People {
  ...
  set(properties, to, callback) {
    this.DATracker.peopleSet(properties, to, callback);
  }
  /**
   * 设置: 姓名
   */
  set_realname(realname) {
    this.DATracker.peopleSet({"$realName" : realname});
  }
  /**
   * 设置: 国家
   */
  set_country(country) {
    this.DATracker.peopleSet({"$country" : country});
  }
  /**
   * 设置：省份
   */
  set_province(province) {
    this.DATracker.peopleSet({"$region" : province});
  }
  /**
   * 设置：城市
   */
  set_city(city) {
    this.DATracker.peopleSet({"$city" : city});
  }
  /**
   * 设置：性别  0-女，1-男，2-未知
   */
  set_gender(gender) {
    if ([0,1,2].indexOf(gender) > -1) {
      this.DATracker.peopleSet({"$gender" : gender});
    }
  }
  /**
   * 设置：生日
  ...
  set_birthday(birthday) {
    if(!_.checkTime(birthday)) {
      return
    }    this.DATracker.peopleSet({"$birthday" : birthday});
  }
  /**
   * 大集合：账户 + 姓名 + 生日 + 性别
   */
  set_populationWithAccount(account, realname, birthday, gender) {
    ...
    //生日合法检测，yy-MM-dd
    this.DATracker.peopleSet({'$account': account, "$realName" : realname, "$birthday": birthday, "$gender": gender});
  }
  /**
   * 国家 + 省份 + 城市
   */
  set_location(country, region, city) {
    ...
    this.DATracker.peopleSet({"$country" : country, "$region": region, "$city": city});
  }
}
...
class DATracker {
  ...
  setSDKAPI(config) {
    ...
    if (miniType === 'my') {
      ...
      this.SDKAPI.setStorageSync = (name, str) => {
        my.setStorageSync({
          key: name,
          data: str
        });
      };
      this.SDKAPI.getStorageSync = (name) => {
    ...
    this.SDKAPI.miniType = miniType;
  }

  constructor(DATrackAgent) {
    this.DATrackAgent = DATrackAgent;
    this.config = {
      // 小程序类型：wx （微信）、tt（字节跳动）、my（支付宝）、swan（百度） 
      miniType: 'wx',
      apiHost: 'https://hubble.netease.com',
      debugHost: 'https://hubble.netease.com',
      appKey: '',
      userId: '',
      sessionStartTime: '',
      persistedTime: '',
  ...
  init(appKey, config) {
    ...
    const DATrackAgent = this.DATrackAgent;
    this.Agent = new DATrackAgent(this, _);
  ...
  */
  ...
  setShareInfo(params, properties) {
    ...
    if (!(params && _.isObject(params.query) && params.query.hubbleshare)) {
      properties = Object.assign(properties, shareObj);
      return;
    }
    hubbleshare = decodeURIComponent(params.query.hubbleshare);
    if (!_.isJSONString(hubbleshare)) {
      properties = Object.assign(properties, shareObj);
      return;
    }    hubbleshare = JSON.parse(hubbleshare);
    const $shareDepth = hubbleshare.$shareDepth;
    const $shareUrlPath = hubbleshare.$shareUrlPath;
    const $firstShareUid = hubbleshare.$firstShareUid;
    const $lastShareUid = hubbleshare.$lastShareUid;
  ...
  getPageParameter() {
    ...
    if (page) {
      ...
      for(let key in params) {
        // hubbleshare 为sdk的自动追踪后续分享的参数，并非用户页面的业务参数，这里要过滤掉。
        if (key !== 'hubbleshare') {
          if (str === '') {
            str = '?' + key + '=' + params[key];
          } else {
  ...
  // 设备激活事件
  daActivate() {
    if (_.localStorage.isFirst) {
        // 从本地获取
        var localDeviceUdid = _.localStorage.get('deviceUdid');
        this.setConfig('deviceUdid', localDeviceUdid );
        this.track('da_activate');
        this.set();
        _.localStorage.isFirst = false;
    }
  }
  // 上报异常日志数据
  daSendError(error) {
...
var load = new DATracker(DATrackAgent);

export default load;
/* 关注Hubble平台:https://hubble.netease.com; 有问题请联系我：hzwangyang1@corp.netease.com  */
